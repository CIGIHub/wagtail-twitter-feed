...
class TestTwitterFeedCreateView(TestCase, WagtailTestUtils):
    ...
    def test_create_invalid(self):
        ...
        self.assertContains(response,
                            "The snippet could not be created due to errors."
                            )
    ...
    def test_create_inactive(self):
        response = self.post(post_data={'screen_name': 'twitter',
                                        'active': 'False'})
        self.assertRedirects(response, reverse('wagtailsnippets_list',
                                               args=('twitter', 'user')))
...
class TestTwitterFeedEditView(TestCase, WagtailTestUtils):
    ...
    def test_non_existant_model(self):
        response = self.client.get(
            reverse('wagtailsnippets_edit',
                    args=('twitter', 'foo', self.test_snippet.id)
                    )
        )
    ...
    def test_nonexistant_id(self):
        response = self.client.get(
            reverse('wagtailsnippets_edit', args=('twitter', 'user', 999999)))
        self.assertEqual(response.status_code, 404)
    ...
    def test_edit(self):
        ...
        snippets = twitter_models.User.objects.filter(
            screen_name='editted_twitter')
        self.assertEqual(snippets.count(), 1)
        self.assertEqual(snippets.first().active, False)
...
class TestTwitterFeedDelete(TestCase, WagtailTestUtils):
    ...
    def test_delete_post(self):
        ...
        self.assertRedirects(
            response,
            reverse('wagtailsnippets_list', args=('twitter', 'user'))
        )

        # Check that the page is gone
        self.assertEqual(
            twitter_models.User.objects.filter(screen_name='twitter').count(),
            0
        )
...
class TestLatestTweetsTemplateTag(TestCase):
    ...
    def test_get_tweets_not_a_number_raises_error(self):
        ...
        self.assertRaisesMessage(TemplateSyntaxError,
                                 "Tag latest_tweets requires a single "
                                 "positive integer argument, given 'Five'.",
                                 twitter_tags.latest_tweets, "Five")

    def test_get_more_tweets_then_exist_returns_all(self):
        tweets = twitter_tags.latest_tweets(200)
        self.assertEqual(len(tweets),
                         twitter_models.Tweet.objects.all().count()
                         )

    def test_get_tweets_negative_number_raises_error(self):
        self.assertRaisesMessage(TemplateSyntaxError,
                                 "Tag latest_tweets requires a single "
                                 "positive integer argument, given -1.",
                                 twitter_tags.latest_tweets, -1)
